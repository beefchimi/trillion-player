...
var isProduction = true,
	...
	sourceMap    = false,
	deployPath   = secrets.server.stage; // secrets.server.prod;
...
gulp.task('images', function() {
	...
	gulp.src(paths.images.src + paths.images.player.body + '*.png')
		...
		.pipe(gulp.dest(paths.images.dest + paths.images.player.body));

	// player face pngs
	gulp.src(paths.images.src + paths.images.player.face + '*.png')
		.pipe(plugins.changed(paths.images.dest + paths.images.player.face))
		.pipe(plugins.imagemin({
			optimizationLevel: 7,
			progressive: true,
			use: [pngcrush()] // very slow, but better compression
		}))
		.pipe(gulp.dest(paths.images.dest + paths.images.player.face));

	// player flair pngs
	gulp.src(paths.images.src + paths.images.player.flair + '*.png')
		.pipe(plugins.changed(paths.images.dest + paths.images.player.flair))
		.pipe(plugins.imagemin({
			optimizationLevel: 7,
			progressive: true,
			use: [pngcrush()] // very slow, but better compression
		}))
		.pipe(gulp.dest(paths.images.dest + paths.images.player.flair));

	// player hair pngs
	gulp.src(paths.images.src + paths.images.player.hair + '*.png')
		.pipe(plugins.changed(paths.images.dest + paths.images.player.hair))
		.pipe(plugins.imagemin({
			optimizationLevel: 7,
			progressive: true,
			use: [pngcrush()] // very slow, but better compression
		}))
		.pipe(gulp.dest(paths.images.dest + paths.images.player.hair));

	// player head pngs
	gulp.src(paths.images.src + paths.images.player.head + '*.png')
		.pipe(plugins.changed(paths.images.dest + paths.images.player.head))
		.pipe(plugins.imagemin({
			optimizationLevel: 7,
			progressive: true,
			use: [pngcrush()] // very slow, but better compression
		}))
		.pipe(gulp.dest(paths.images.dest + paths.images.player.head));

	// player mouth pngs
	gulp.src(paths.images.src + paths.images.player.mouth + '*.png')
		.pipe(plugins.changed(paths.images.dest + paths.images.player.mouth))
		.pipe(plugins.imagemin({
			optimizationLevel: 7,
			progressive: true,
			use: [pngcrush()] // very slow, but better compression
		}))
		.pipe(gulp.dest(paths.images.dest + paths.images.player.mouth));
